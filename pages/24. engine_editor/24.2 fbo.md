...
那现在使用了OpenGL的FBO，创建了240x160大小的RenderTexture，那么片段处理器就只需要处理240x160次了。

这个性能提升怎么样呢？

下图是960x640的原生分辨率RenderTexture下，GPU在6.1&-7.4%之间浮动。

![](../../imgs/engine_editor/draw_rtt/960_640_rtt_gpu.gif)

下图是240x160的超低分辨率RenderTexture下，GPU在4.7&-6.2%之间浮动。

![](../../imgs/engine_editor/draw_rtt/240_160_rtt_gpu.gif)

提升巨大！

当然了，FBO的颜色数据最终还是要送到系统默认的帧缓冲区去的，要呈现在显示器中。

这个时候240x160的数据，要显示到960x640的屏幕上，就会模糊了。
...
上面我们将游戏场景渲染到240x160的RenderTexture上，然后放到到960x640，性能得到极大提升，但是画面变得特别模糊。

我之前用过一些人工智能放大图片的软件，可以将低分辨率图片，放到至2x、4x大小，而保持一定的清晰度。
...
而DLSS可以将240x160的RenderTexture，借助AI技术，将其放大到960x640，甚至1920x1080。
