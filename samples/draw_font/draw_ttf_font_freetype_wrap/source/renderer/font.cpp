...
Font* Font::LoadFromFile(std::string font_file_path,unsigned short font_size){
    ...
    font->font_texture_=Texture2D::Create(font->font_texture_size_,font->font_texture_size_, GL_RGBA,GL_ALPHA,GL_UNSIGNED_BYTE,pixels);
    delete pixels;
...
void Font::LoadCharacter(char ch) {
    ...
    font_texture_->UpdateSubImage(font_texture_fill_x, font_texture_fill_y, ft_bitmap.width, ft_bitmap.rows,  GL_RGBA, GL_UNSIGNED_BYTE, ft_bitmap.buffer);
